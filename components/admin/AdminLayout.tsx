...
const AdminLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const menuItems = [
    ...
    { label: 'Promos', icon: ShoppingBag, path: '/admin/promos' },
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 transition-transform duration-300 transform lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
